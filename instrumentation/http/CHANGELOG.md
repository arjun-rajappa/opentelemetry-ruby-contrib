...
* [#1547](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1547) - feat: http semconv opt in files
  ...
  While this approach introduces multiple new and similar files and tests (potentially making debugging and maintenance more difficult), the tradeoff ensures we're only patching the HTTP class once and makes the eventual cleanup of the old conventions mostly a matter of deleting files and small refactors. [Another approach](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1166) (also taken by Python) introduces a `StabilityMode` class that checks calls to determine which attributes to send, however this approach adds complexity of calling a method every time an attribute is added. 
  
  Hopefully with the temporary nature of this work, this solution can be seen as a way to start the transition. The approach can also be easily duplicated to other HTTP libraries and support [the similar DB migration](https://opentelemetry.io/docs/specs/semconv/non-normative/db-migration/).
...
* [#1570](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1570) - fix: update span name when semconv stability is enabled
  [Semantic conventions 1.34.0 updates span names](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/#http-server-span-name) to drop the leading `HTTP`, for example `HTTP GET` becomes just `GET`. To facilitate the transition from old to new HTTP semantic conventions, a new stability mode was introduced in #1547. However, the PR did not account for the span name update.
...
* [#1579](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1579) - release: Release opentelemetry-instrumentation-http 0.25.1 (was 0.25.0)
  *  **opentelemetry-instrumentation-http 0.25.1** (was 0.25.0)
...
* Co-authored-by: release: Release opentelemetry-instrumentation-http 0.25.1 (was 0.25.0) ([#1579](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1579) [28c61be](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/28c61be5004d62255c6b77112bb9b57218b80d7a))
  *  **opentelemetry-instrumentation-http 0.25.1** (was 0.25.0)

* Co-authored-by: release: Release opentelemetry-instrumentation-http 0.25.1 (was 0.25.0) ([#1579](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1579) [28c61be](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/28c61be5004d62255c6b77112bb9b57218b80d7a))
  *  **opentelemetry-instrumentation-http 0.25.1** (was 0.25.0)

* release: release: Release opentelemetry-instrumentation-http 0.25.1 (was 0.25.0) ([#1579](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1579) [28c61be](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/28c61be5004d62255c6b77112bb9b57218b80d7a))
  *  **opentelemetry-instrumentation-http 0.25.1** (was 0.25.0)

* fix: update span name when semconv stability is enabled ([#1570](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1570) [3a237a5](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/3a237a5e089239623ee8cfa2e8d2b7e07e1d552a))
  [Semantic conventions 1.34.0 updates span names](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/#http-server-span-name) to drop the leading `HTTP`, for example `HTTP GET` becomes just `GET`. To facilitate the transition from old to new HTTP semantic conventions, a new stability mode was introduced in #1547. However, the PR did not account for the span name update.
  
  If `OTEL_SEMCONV_STABILITY_OPT_IN` is set to either `http` or `http/dup`, the span name should be inline with the new conventions. This PR makes this update and drops the leading `HTTP` from span names.

* **FIX:** fix: update span name when semconv stability is enabled ([#1570](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1570) [3a237a5](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/3a237a5e089239623ee8cfa2e8d2b7e07e1d552a))
  [Semantic conventions 1.34.0 updates span names](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/#http-server-span-name) to drop the leading `HTTP`, for example `HTTP GET` becomes just `GET`. To facilitate the transition from old to new HTTP semantic conventions, a new stability mode was introduced in #1547. However, the PR did not account for the span name update.
  
  If `OTEL_SEMCONV_STABILITY_OPT_IN` is set to either `http` or `http/dup`, the span name should be inline with the new conventions. This PR makes this update and drops the leading `HTTP` from span names.

* feat: http semconv opt in files ([#1547](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1547) [7837db8](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/7837db80a55a5b840d9d38f1d014536f104f9132))
  This PR is intended to assist in the transition from the old to new HTTP semantic conventions. Per the [HTTP semantic convention stability migration spec](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/), users should be able to set the environment variable `OTEL_SEMCONV_STABILITY_OPT_IN` to:
  - `http` to emit stable conventions only
  - `http/dup` to emit both old and the stable conventions
  -  a blank env var will emit old conventions only
...
* **NEW:** feat: http semconv opt in files ([#1547](https://github.com/open-telemetry/opentelemetry-ruby-contrib/pull/1547) [7837db8](https://github.com/open-telemetry/opentelemetry-ruby-contrib/commit/7837db80a55a5b840d9d38f1d014536f104f9132))
  This PR is intended to assist in the transition from the old to new HTTP semantic conventions. Per the [HTTP semantic convention stability migration spec](https://opentelemetry.io/docs/specs/semconv/non-normative/http-migration/), users should be able to set the environment variable `OTEL_SEMCONV_STABILITY_OPT_IN` to:
  - `http` to emit stable conventions only
  - `http/dup` to emit both old and the stable conventions
  -  a blank env var will emit old conventions only
